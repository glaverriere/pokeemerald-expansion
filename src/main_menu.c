 ...
 * Task_NewGameBirchSpeech_StartBirchSpoinkPlatformFade
 * Task_NewGameBirchSpeech_StartBirchSpoinkPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchSpoink
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForSpoink
 * Task_NewGameBirchSpeechSub_WaitForSpoink
...
static void Task_NewGameBirchSpeechSub_WaitForSpoink(u8);
static void Task_NewGameBirchSpeech_StartBirchSpoinkPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchSpoink(u8);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tSpoinkSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_SPOINK);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForSpoink;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForSpoink(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tSpoinkSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchSpoinkPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchSpoinkPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tSpoinkSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tSpoinkSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tSpoinkSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchSpoink;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchSpoink(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tSpoinkSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tSpoinkSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tSpoinkSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateSpoinkSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_SPOINK, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    spoinkSpriteId = NewGameBirchSpeech_CreateSpoinkSprite(100, 0x4B);
    gSprites[spoinkSpriteId].callback = SpriteCB_Null;
    gSprites[spoinkSpriteId].oam.priority = 0;
    gSprites[spoinkSpriteId].invisible = TRUE;
    gTasks[taskId].tSpoinkSpriteId = spoinkSpriteId;
